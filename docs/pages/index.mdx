...
It allows users to sign multiple EIP-712 messages with a single, verifiable signature. This eliminates consecutive off-chain signature prompts, reducing friction and boosting UX.

## Motivation

Off-chain signatures are the hallmark of modern Ethereum apps: Hyperliquid, Wallet Sign In, Payments, UniswapV4 (Permit2), etc. They allow developers to safely interact with EOAs without relying on state-modification, which is often costly, slow, and requires the EOA to hold ETH.
...
Messages can be verified in isolation or aggregate. Apps can require that message (`x`) is only valid when signed in combination message (`y`).
